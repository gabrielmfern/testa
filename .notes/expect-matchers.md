...
All of section 1 (core matchers) plus the `.not` modifier, implemented in Zig in
`src/main.zig`. `expect(actual)` returns a wrapper whose prototype carries every
matcher (and a `.not` twin backed by a negated prototype); each matcher reads
`actual` off the receiver and throws an Error on mismatch. The two prototypes are
built once at startup, so an assertion costs one object, not one function per
matcher.

Still missing: `.resolves`/`.rejects` (section 2), snapshots (3), mock matchers
(4), asymmetric matchers (5), and the `expect.*` statics (6).

## Implementation note (read before the lists)

The matchers live in Zig, not a JS prelude. `runMatcher(comptime kind, comptime
negate, info)` holds the logic for all of them as one big switch; `makeMatcher`
generates a thin v8 callback trampoline per (matcher, negate) pair at comptime.
The C++ shim (`src/node_embed.cpp`) only gained thin 1:1 v8 primitives the logic
needs — type predicates (`IsArray`/`IsObject`/...), coercions (`BooleanValue`,
`NumberValue`, `TypeOf`, `InstanceOf`), object/array access, and
`Object::New`-with-prototype — keeping with the rule that the shim stays
mechanical and orchestration stays in Zig.

`deepEqual` is the recursive equality walk shared by `toEqual`/`toStrictEqual`/
`toContainEqual`/`toBeOneOf` and (for leaves) `toMatchObject`. It handles
primitives, arrays, plain objects, `Date`, `RegExp`, `Map`, and `Set`. The
asymmetric-matcher hook (section 5) is not wired in yet.
...
- ✅ `toStrictEqual(value)` — deep + checks class, `undefined` keys
...
- ✅ `toContain(item)` — string substring, array/Set member (`Object.is`-ish)
- ✅ `toContainEqual(item)` — array/Set member by deep equality
